...
import zoraiz.fast_past_papers.DownloadTasks.DownloadTask;
...
public class DownloadFile extends AppCompatActivity implements View.OnClickListener {
        ...
        switch (view.getId()) {
            case R.id.downloadPdf:
                if (isConnectingToInternet())
                    new DownloadTask(DownloadFile.this, downloadPdf, Utils.downloadPdfUrl);
                else
                    Toast.makeText(DownloadFile.this, "Oops!! There is no internet connection. Please enable internet connection and try again.", Toast.LENGTH_SHORT).show();
                break;
            case R.id.downloadDoc:
                if (isConnectingToInternet())
                    new DownloadTask(DownloadFile.this, downloadDoc, Utils.downloadDocUrl);
                else
                    Toast.makeText(DownloadFile.this, "Oops!! There is no internet connection. Please enable internet connection and try again.", Toast.LENGTH_SHORT).show();
                break;
            case R.id.downloadZip:
                if (isConnectingToInternet())
                    new DownloadTask(DownloadFile.this, downloadZip, Utils.downloadZipUrl);
                else
                    Toast.makeText(DownloadFile.this, "Oops!! There is no internet connection. Please enable internet connection and try again.", Toast.LENGTH_SHORT).show();
                break;
            case R.id.downloadVideo:
                if (isConnectingToInternet())
                    new DownloadTask(DownloadFile.this, downloadVideo, Utils.downloadVideoUrl);
                else
                    Toast.makeText(DownloadFile.this, "Oops!! There is no internet connection. Please enable internet connection and try again.", Toast.LENGTH_SHORT).show();
                break;
            case R.id.downloadMp3:
                if (isConnectingToInternet())
                    new DownloadTask(DownloadFile.this, downloadMp3, Utils.downloadMp3Url);
                else
                    Toast.makeText(DownloadFile.this, "Oops!! There is no internet connection. Please enable internet connection and try again.", Toast.LENGTH_SHORT).show();
                break;
    ...
    private void openDownloadedFolder() {
        ...
        if (new CheckForSDCard().isSDCardPresent()) {
            ...
            if (!apkStorage.exists())
                Toast.makeText(DownloadFile.this, "Right now there is no directory. Please download some file first.", Toast.LENGTH_SHORT).show();
            ...
            Toast.makeText(DownloadFile.this, "Oops!! There is no SD Card.", Toast.LENGTH_SHORT).show();
